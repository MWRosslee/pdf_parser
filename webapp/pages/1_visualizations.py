...
source_df = st.session_state.get("edited_df", st.session_state.get("df"))
...
if source_df is not None:
    ...
    if not required_columns.issubset(df.columns):
        st.info("Cash-flow visualizations need the Date and Amount columns to be included in the review table.")
    else:
        ...
        if df.empty:
            st.info("No dated transactions are available to visualize.")
        else:
            df.index = df["date"]
            df["Income"] = df["amount"].apply(lambda x: max(0, x))
            df["Expenses"] = df["amount"].apply(lambda x: abs(x) if x < 0 else 0)
            df = df.drop(columns=[column for column in ["description", "date", "bank"] if column in df.columns])
