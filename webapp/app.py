...
from webapp.helpers import create_df, parse_bank_statement, render_parsing_preview, show_df
...
def app() -> pd.DataFrame:
    ...
    if files:
        ...
        if documents:
            df = render_parsing_preview(documents)
...
def prepare_documents(uploaded_files: list[UploadedFile]) -> list[PdfDocument]:
    documents = []
